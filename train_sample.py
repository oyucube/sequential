...
from data.libtraindata import make_train_data
import time
import importlib
...
parser.add_argument("-g", "--gpu", type=int, default=-1,
                    help="use gpu")
# train id
parser.add_argument("-i", "--id", type=str, default="test",
                    help="data id")
parser.add_argument("-a", "--am", type=str, default="dram2",
...
if n_data == -1:
    n_data = int(data_max)
acc_array = np.zeros(n_epoch)
err_array = np.zeros(n_epoch)
lac_array = np.zeros(n_epoch)
max_acc = 0
...
for epoch in range(n_epoch):
    ...
    for i in tqdm(range(0, n_data - test_b, train_b), ncols=60):
        ...
        # 順伝播
        model.cleargrads()
        loss_func = model(x, t, num_lm * train_b, batch_size=train_b, train=1, n_step=ns)
        loss_func.backward()
    ...
    # 順伝播
    model.reset()
    acc, y, lac = model(x, t, test_b, train=0, n_step=num_step)

    # 分類精度の記録
    for i in range(0, test_b):
        if xp.argmax(y[i]) == xp.argmax(t.data[i]):
            err_array[epoch] += 1
    err_array[epoch] = 1 - err_array[epoch] / test_b
    acc_array[epoch] = acc
    lac_array[epoch] = lac
    print("accuracy:{:1.4f} err:{:1.4f} last acc:{:1.4f}".format(acc_array[epoch], err_array[epoch], lac_array[epoch]))
    ...
    with open(log_filename, mode='a') as fh:
        fh.write("accuracy:{:1.4f} err:{:1.4f} last acc:{:1.4f}\n".format(acc_array[epoch], err_array[epoch], lac_array[epoch]))

    np.save(log_dir + "/acc.npy", acc_array)
    np.save(log_dir + "/err.npy", err_array)
    np.save(log_dir + "/lac.npy", lac_array)
    # モデルの保存
    if gpu_id >= 0:
        model.to_cpu()
...
with open(log_filename, mode='a') as fh:
    fh.write("last acc:{}  max_acc:{}\n".format(acc_array[n_epoch - 1], max_acc))
